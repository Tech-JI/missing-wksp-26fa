...
\begin{frame}[fragile]{Extension marketplace}
	\begin{itemize}
		\item Access thousands of extensions through the Extensions view (Cmd+Shift+X)
...
\begin{frame}[fragile]{macOS keyboard shortcuts}
	\begin{itemize}
		\item \texttt{Cmd+Shift+P}: command palette
		\item \texttt{Cmd+P}: quick open files
		\item \texttt{Cmd+Shift+X}: extensions
		\item \texttt{Cmd+B}: toggle sidebar
		\item \texttt{Cmd+,}: settings
		\item \texttt{Ctrl+\textasciigrave}: integrated terminal (terminal stays Ctrl on macOS!)
		\item \texttt{Cmd+K Cmd+S}: shortcut editor
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Recommended extensions}
	\begin{itemize}
		...
		\item \textbf{Remote - SSH}: connect to remote servers, lab machines, or your workstation right from VS Code
